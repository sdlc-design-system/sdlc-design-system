...
class Sidebar extends React.Component {
  ...
  render() {
    loop1: for (let i = 0; i < this.props.list.length; i++) {
      //loop thru the list
      if (this.props.list[i].length > 2) {
        //searching for only the headings that have subheadings
        for (let j = 1; j < this.props.list[i].length; j++) {
          //loop through the subheadings
          if (this.props.activeTab === this.props.list[i][j][0]) {
            //if we've arrived on a heading that contains subheadings,
            if (this.state[this.props.list[i][0][0]] === false) {
              //and the accordion is currently marked unopened (false),
              this.setState({
                ...this.reset,
                [this.props.list[i][0][0]]: true
              }); //go ahead and mark it open (true)
    ...
    return (
      <div className="sideVerticalLine disappearingSideBar">
        {this.props.list.map(x =>
          typeof x[0] === 'string' ? ( //if there are no subheadings, do this section
            <div
              key={x[0]}
              className={this.props.activeTab === x[0] ? 'active' : ''}>
              <a
                href={'#' + x[0]}
            ...
            <span key={x[0][0]}>
              <div
                ...
                <a
                  ...
                  className={
                    this.props.activeTab === x[0][0] ? 'highlight' : ''
                  }>
                  {x[0][1]}
                </a>
              </div>
              ...
              x
                ...
                ? x.map((y, index) =>
                    index > 0 ? ( //if we should show the open accordion, go ahead and map through it, starting with the 2nd thing (the first thing was the heading itself)
                      <div key={y[0]} className="indentSidebar">
                        <div
                          ...
                          <a
                            ...
                            className={
                              this.props.activeTab === y[0] ? 'highlight' : ''
                            }>
                            {y[1]}
                          </a>
                        </div>
